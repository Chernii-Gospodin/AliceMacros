...
def check_user_message(func):
    global BUCKET_NAME
    def wrapper(original_utterance, USER_ID, s3, session_state):
        ...
        if 'Contents' in macros:
            text, session_state['user_action'], session_state['user_links'], buttons_status = func(macros_name, USER_ID, s3, session_state)
        else:
...
def create_macros(original_utterance, USER_ID, s3, session_state, user_links):
    global BUCKET_NAME
    if user_links == 'false':
...
@check_user_message
def change_macros_links_false(macros_name, USER_ID, s3, session_state):
    global BUCKET_NAME
...
def change_macros_links_true(original_utterance, USER_ID, s3):
    global BUCKET_NAME
    user_macroses = s3.list_objects(Bucket=BUCKET_NAME, Prefix=USER_ID)
    for macros in user_macroses['Contents']:
        if macros['Size'] == 0:
...
@check_user_message
def use_macros(macros_name, USER_ID, s3, session_state):
    global BUCKET_NAME
    macros = s3.get_object(Bucket=BUCKET_NAME, Key=USER_ID+'/'+macros_name+'.txt')
...
async def handler(event, context):
    global BUCKET_NAME, BUTTONS_ENABLED
...
\n\nВот твой USER_ID, необходимый для приложения: " + USER_ID + "\nПодготовте нужные ссылки и пути заранее, так как Алиса сбрасывается, если нажать в браузере на что-то кроме неё"
    ...
    if 'user_action' in event['state']['session']:
        ...
        if event['state']['session']['user_action'] == 'Create':
            text, session_state['user_action'], session_state['user_links'], buttons_status = create_macros(event['request']['original_utterance'], USER_ID, s3, session_state, event['state']['session']['user_links'])
        
        if event['state']['session']['user_action'] == 'Use':
            text, session_state['user_action'], session_state['user_links'], buttons_status = use_macros(event['request']['original_utterance'], USER_ID, s3, session_state)

        if event['state']['session']['user_action'] == 'Change':
            if event['state']['session']['user_links'] == 'false':
                text, session_state['user_action'], session_state['user_links'], buttons_status = change_macros_links_false(event['request']['original_utterance'], USER_ID, s3, session_state)
            elif event['state']['session']['user_links'] == 'true':
                text = change_macros_links_true(event['request']['original_utterance'], USER_ID, s3)
